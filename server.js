...
gptConnection.post("/ask-gpt-polishing", async (req, res) => {
...
app.use(async ctx => {
    ...
    if(ctx.url === '/save-friendList' && ctx.method === 'POST'){
    ...
    if(ctx.url === '/get-friendlist' && ctx.method === 'POST'){
        const { username } = ctx.request.body
        const result = await Friend.findOne({
            where: {
                username: username
    ...
    if(ctx.url === '/save-mymessage' && ctx.method === 'POST'){
    ...
    if(ctx.url === '/get-message' && ctx.method === 'POST'){
    ...
    if (ctx.url === '/save-chat-style' && ctx.method === 'POST') {
    ...
    if (ctx.url === '/get-settings' && ctx.method === 'POST') {
        const { username, friendname } = ctx.request.body
